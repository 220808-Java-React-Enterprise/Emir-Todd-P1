...
//            byte[] blobAsBytes = obj.getReceipt().getBytes(1, blobLength);
    ...
    public Reimbursement getByRequestId(String id) {
        try (Connection con = ConnectionFactory.getInstance().getConnection()) {
            ...
            if (rs.next()) {
                return new Reimbursement(rs.getString("reimb_id"), rs.getDouble("amount"), rs.getTimestamp("submitted"), rs.getTimestamp("resolved"), rs.getString("description"), rs.getBytes("receipt"), rs.getString("payment_id"), rs.getString("author_id"), rs.getString("resolver_id"), rs.getString("status_id"), rs.getString("type_id"));
            }
